...
for i in range(10000):
    for batch_no in range(3, 18):
        ...
        if g_loss < 1.7:
            _, _, g_loss, g_reg, d_loss, audio_out   = sess.run(
                    [g_optim, d_optim, gan.g_loss, gan.g_reg, gan.d_loss, gan.gen_audio],
                    feed_dict = {
                        gan.video : videos,
        ...
        for _ in range(1):
            _, g_loss, g_reg, audio_out   = sess.run(
                        [g_optim, gan.g_loss, gan.g_reg, gan.gen_audio],
                        feed_dict = {
                            gan.video : videos,
                            gan.wrong_video : wrongvideos,
                            gan.real_audio : audio_clip,
        ...
        if batch_no % 3 == 0:
            audio_out = audio_out * 26600
            audio_out = audio_out.astype('int16')
            for idx in range(10):
                try:
                    write('test_generated' + str(idx) + '.wav', 8000, audio_out[idx,:,:])
                except:
                    pass
