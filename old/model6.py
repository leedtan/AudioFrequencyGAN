...
def generate_gray_noise_audio(transform):
    ...
    random_mult = tf.random_uniform([bs,1,1],0,1)
...
class GAN:
    ...
    def add_residual_pre(self, prev_layer, k_h = 10, k_w = 2, hidden_filters = None,name_func=None):
        ...
        residual = ops.deconv2d(ops.lrelu(bn1(low_dim)), 
            [self.options['batch_size'], s, 2, filters], k_h=k_h, k_w=k_w, d_h = 2, d_w = 1, name=name_func())
    ...
    def build_model(self):
        ...
        self.z_noise = tf.placeholder('float32', [bs,100 ])
        self.gray_noise = generate_gray_noise_audio(self.real_audio)
        
        self.gen_audio = self.generator(self.z_noise, vid)
        self.half_audio = (self.real_audio + self.gen_audio)/2
        ...
        self.d_loss_real = tf.reduce_mean(c_e(real_img_logit, pos_ex)) + tf.reduce_mean(c_e(real_txt_logit, pos_ex))*3
        ...
        self.d_loss_half = tf.reduce_mean(c_e(half_img_logit, pos_ex)) + tf.reduce_mean(c_e(half_txt_logit, pos_ex))
        ...
        self.g_loss = tf.reduce_mean(c_e(gen_img_logit, pos_ex)) + tf.reduce_mean(c_e(gen_txt_logit, pos_ex)) + \
                tf.reduce_mean(tf.square(self.gen_audio - self.real_audio)) * .01 + \
                tf.reduce_mean(tf.square(real_acts - gen_acts))
        
        d_loss = self.d_loss_real + self.d_loss_wrong + self.d_loss_gen + self.d_loss_gray + self.d_loss_half/100
        self.d_loss = d_loss
    ...
    def generator(self, z_noise, video):
        ...
        a_g_dim  = 8000
        a = a_g_dim
        a8, a4, a2 = int(a/8), int(a/4), int(a/2)
        vid = ops.lrelu(bnv1(ops.conv2d(video, 100, name = self.g_name())))
        vid = ops.lrelu(bnv2(ops.conv2d(vid, 100, name = self.g_name())))
        vid = ops.lrelu(bnv3(ops.conv2d(vid, 100, name = self.g_name())))
        hidden = reshape(vid, [bs, -1])
        hidden = concat(1, [hidden, z_noise])
        audio = ops.lrelu(bnl1(ops.linear(hidden, a4 * 10, self.g_name())))
        audio = reshape(audio, [bs, a8, 2, 10])
        g_dim = 100
        audio = ops.lrelu(bna1(ops.deconv2d(audio, [bs, a4, 2, g_dim],k_h = 100, k_w = 2, d_h=2, d_w=1, name=self.g_name())))
        audio = self.add_residual_pre(audio, name_func = self.g_name)
        audio = ops.lrelu(bna2(ops.deconv2d(audio, [bs, a2, 2, g_dim],k_h = 100, k_w = 2, d_h=2, d_w=1, name=self.g_name())))
        audio = self.add_residual_pre(audio, name_func = self.g_name)
        audio = ops.lrelu(bna3(ops.deconv2d(audio, [bs, a, 2, 1],k_h = 100, k_w = 2, d_h=2, d_w=1, name=self.g_name())))
        audio = self.add_residual_pre(audio, name_func = self.g_name)
        audio = ops.deconv2d(audio, [bs, a, 2, 1],k_h = 10, k_w = 2, d_h=1, d_w=1, name=self.g_name())
        audio = reshape(audio, [bs, a, 2])
        #audio = self.blur_audio(audio)
        return tf.tanh(audio)
    ...
    def discriminator(self, audio, video, reuse=False):
        ...
        a1 = ops.lrelu(ops.conv2d(audio, ddim, k_h = 100, k_w = 2, d_h = 4, d_w = 1, name='d_a1'))
        a2 = ops.lrelu(ops.conv2d(a1, ddim*2, k_h = 60, k_w = 2, d_h = 4, d_w = 1, name='d_a2'))
        a3 = ops.lrelu(ops.conv2d(a2, ddim*4, k_h = 40, k_w = 2, d_h = 4, d_w = 1, name='d_a3'))
        
        a_flat = reshape(a3, [bs, -1])
        
        v1 = ops.lrelu(ops.conv2d(video, 40, name='d_v1'))
        v2 = ops.lrelu(ops.conv2d(v1, 60, name='d_v2'))
        
        v_flat = reshape(v2, [bs, -1])
        
        av_flat = concat(1, [a_flat, v_flat])
        
        a_hidden = ops.lrelu(ops.linear(a_flat, 100, 'd_ah'))
        a_out = ops.linear(a_hidden, 1, 'd_aout')
        
        av_hidden = ops.lrelu(ops.linear(av_flat, 100, 'd_avh'))
