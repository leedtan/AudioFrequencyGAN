...
class GAN:
    ...
    def build_model(self):
        ...
        self.g_reg = tf.reduce_mean(tf.square(self.gen_audio - self.real_audio)) * .01 + \
                tf.reduce_mean(tf.square(real_acts - gen_acts)) * 100
    ...
    def generator(self, z_noise, video):
        ...
        a_g_dim  = 8000
        a = a_g_dim
        a64, a32, a16, a8, a4, a2 = int(a/64), int(a/32), int(a/16), int(a/8), int(a/4), int(a/2)
        ...
        audio = ops.lrelu(bnl1(ops.linear(hidden, a32 * 10, self.g_name())))
        audio = reshape(audio, [bs, a64, 2, 10])
        g_dim = 100
        audio = ops.lrelu(bna1(ops.deconv2d(audio, [bs, a16, 2, g_dim*2],k_h = 20, k_w = 2, d_h=4, d_w=1, name=self.g_name())))
        audio = self.add_residual_pre(audio, k_h = 20, name_func = self.g_name)
        audio = ops.lrelu(bna2(ops.deconv2d(audio, [bs, a4, 2, g_dim],k_h = 40, k_w = 2, d_h=4, d_w=1, name=self.g_name())))
        audio = self.add_residual_pre(audio, k_h = 40, name_func = self.g_name)
        audio = ops.lrelu(bna3(ops.deconv2d(audio, [bs, a, 2, 1],k_h = 80, k_w = 2, d_h=4, d_w=1, name=self.g_name())))
        audio = self.add_residual_pre(audio, k_h = 80, name_func = self.g_name)
        
        audio = ops.deconv2d(audio, [bs, a, 2, 1],k_h = 100, k_w = 2, d_h=1, d_w=1, name=self.g_name())
        audio = self.add_residual_pre(audio, k_h = 40, name_func = self.g_name)
