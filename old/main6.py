...
for i in range(10000):
    for batch_no in range(3, 18):
        ...
        _, _, g_loss, d_loss, audio_out   = sess.run(
        ...
        print('epoch: ', i, 'batch: ', batch_no, 'g_loss:', g_loss, 'd_loss', d_loss)
        for _ in range(1):
            _, g_loss, audio_out   = sess.run(
                        [g_optim, gan.g_loss, gan.gen_audio],
                        feed_dict = {
                            gan.video : videos,
                            gan.wrong_video : wrongvideos,
                            gan.real_audio : audio_clip,
